...
var word = []
var neightbours = dice
var clickDice = (val) => {
    
    if(neightbours.includes(val) && !word.includes(val)) {
        val.classList.toggle("selected")
        neightbours = findNeighbour(dice, val)
        word.push(val)

        input.innerHTML += val.classList[2]
    }

}
dice.forEach(val => val.onclick = () => {
    clickDice(val)
})
...
var cancelWord = () => {
    ...
    neightbours = dice
    word = []
    dice.forEach(d => d.classList.remove('selected'))
}
